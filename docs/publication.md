...
On 2026-09-14, the signed-in James Doss account opened the existing publisher, node page, and 0.6.0 **More** panel. That panel exposed download, delete, and deprecation controls, but no rejection reason or appeal instructions. The specific-version API also omitted `status_reason`. An existing masked publisher key is listed; its value and GitHub secret configuration remain unverified. Review reason **not supplied**, resolution **not verified**. No 0.7.0 Registry upload has been made.

The next required evidence is the Registry review notice or a clarification from the Registry reviewers for version ID `12cb5657-b291-4cef-aeaa-b341300812fa`. The confirmed cross-platform Save Image fix in this branch is not asserted to be the cause of the ban.
...
5. Manually run **Publish tested release**, specifying the existing tag. The workflow resolves and validates its commit, reruns Windows/Linux and native checks for that revision, builds the installable ZIP, publishes the tracked source through Comfy CLI, and waits for active Registry status. It compares every runtime file in the downloaded Registry archive with the tested checkout, then installs that archive in clean ComfyUI and runs the model-free examples before creating the GitHub release. Pending review stops delivery; rerunning resumes the status check without uploading an existing version again.
