# Node Index

This file tracks public Doss Node Suite nodes.

## Released Nodes

| Node Class | Display Name | Category | Purpose |
| --- | --- | --- | --- |
| `DossImageComparer` | `Doss Image Comparer` | `Doss Node Suite / Image` | Compare two IMAGE inputs visually while passing IMAGE tensors through. |
| `DossSaveImage` | `Doss Save Image` | `Doss Node Suite` | Save IMAGE batches to the ComfyUI output folder or an output subfolder while passing the original batch through. |
| `DossWorkflowTimerAndAlarm` | `Doss Workflow Timer and Alarm` | `Doss Node Suite` | Display a live workflow timer on the canvas and optionally play a completion alarm. |

## Status Values

- Planned
- In Progress
- Released
- Deprecated
