...
import { CATEGORIES, matches, captureValues, applyValues, transaction, type Catalog, type CatalogNode, type Preferences, type UserPreset } from "./hub_model.js";

declare const LiteGraph: any;
type GraphNode = any;
const PREFS_EMPTY: Preferences = {schema_version: 1, favorites: [], presets: []};
let catalog: Catalog;
let preferences: Preferences = structuredClone(PREFS_EMPTY);
let objectInfo: Record<string, unknown> = {};
let activeTab = "Tools", query = "", favoritesOnly = false;
let root: HTMLElement | null = null;
let body: HTMLElement, statusLine: HTMLElement;
...
function showError(error: unknown) { message(error instanceof Error ? error.message : String(error), true); }
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  const response: Response = await api.fetchApi(path, init);
  if (!response.ok) {
...
function centerPosition(): [number, number] {
...
function addNode(id: string, values?: UserPreset["values"]) {
  const node = LiteGraph.createNode(id);
  if (!node) throw new Error(`Node ${id} is unavailable. Restart ComfyUI after updating Doss.`);
  transaction(app.graph, () => {
    node.pos = centerPosition();
    app.graph.add(node);
    if (values) applyValues(node, values);
    app.canvas.selectNode?.(node);
  });
  message(`${node.title || id} added. Undo with Ctrl/Cmd+Z.`);
  return node;
}
async function insertExample(file: string) {
  const workflow = await request<any>(`/doss/hub/examples/${encodeURIComponent(file)}`);
  const records = workflow.nodes || [];
  if (!records.length) throw new Error("This example contains no nodes.");
  const prepared: {record: any; node: GraphNode}[] = [];
  for (const record of records) {
    const node = LiteGraph.createNode(record.type);
    if (!node) throw new Error(`Example requires ${record.type}. Install the listed dependency before inserting it.`);
    prepared.push({record, node});
  }
  const [ox, oy] = centerPosition();
  const minX = Math.min(...records.map((n: any) => n.pos[0])), minY = Math.min(...records.map((n: any) => n.pos[1]));
  transaction(app.graph, () => {
    ...
    try {
      for (const {record, node} of prepared) {
        ...
        for (const input of configuration.inputs || []) input.link = null;
        for (const output of configuration.outputs || []) output.links = null;
      ...
      for (const link of workflow.links || []) {
        if (!map.get(link[1])?.connect(link[2], map.get(link[3]), link[4])) throw new Error("An example connection could not be restored.");
      }
      app.canvas.selectNodes?.(prepared.map(item => item.node));
    } catch (error) {
      for (const {node} of prepared) if (node.graph) app.graph.remove(node);
      throw error;
    }
  });
  message(`Inserted ${prepared.length} example nodes. Your existing workflow is preserved; Ctrl/Cmd+Z undoes insertion.`);
}
function selectedNode(): GraphNode {
  const nodes = Object.values(app.canvas?.selected_nodes || {}) as GraphNode[];
  if (nodes.length !== 1 || !catalog.nodes.some(n => n.id === (nodes[0].comfyClass || nodes[0].type))) throw new Error("Select exactly one Doss node on the canvas first.");
  return nodes[0];
}
function portList(label: string, ports: string[]) {
  const block = el("div", "doss-ports");
  block.append(el("strong", "", label), el("span", "", ports.length ? ports.join(" · ") : "None"));
  return block;
}
function nodeCard(node: CatalogNode): HTMLElement {
  ...
  const favorite = button(favored ? "★" : "☆", async () => {
    ...
    await savePreferences(); renderBody();
  }, "doss-favorite");
  ...
  if (!available) add.title = "Node unavailable: restart ComfyUI after installing the update.";
  actions.append(add);
  if (node.example) actions.append(button("Example", () => insertExample(node.example!)));
  ...
  for (const preset of node.presets) help.append(button(`Add · ${preset.name}`, () => addNode(node.id, preset.values)));
...
function toolsView() {
  ...
  if (!filtered.length) { body.append(el("p", "doss-empty", favoritesOnly ? "Favorite tools with the star beside their name to keep them here." : "No tools match. Try image, mask, trim, seed, or batch.")); return; }
...
function workflowsView() {
  ...
  for (const item of catalog.workflows) {
    if (query && !`${item.name} ${item.description} ${item.category}`.toLowerCase().includes(query.toLowerCase())) continue;
    const card = el("article", "doss-card");
  ...
  name.placeholder = "Group title"; name.setAttribute("aria-label", "Group title"); name.value = "Doss · Working group";
  organizer.append(name, button("Group selected nodes", () => {
    const nodes = Object.values(app.canvas?.selected_nodes || {}) as GraphNode[];
    if (!nodes.length) throw new Error("Select nodes on the canvas first.");
    transaction(app.graph, () => {
      ...
      group.pos = [x, y]; group.size = [right - x, bottom - y]; group.color = "#397b70";
      app.graph.add(group);
    });
    message("Group created. Ctrl/Cmd+Z undoes this change."); renderBody();
  }));
...
function presetsView() {
  ...
  const name = el("input", "doss-input");
  name.placeholder = "Preset name"; name.maxLength = 128; name.setAttribute("aria-label", "Preset name");
  body.append(name, button("Save selected node as preset", async () => {
    const node = selectedNode(), title = name.value.trim();
    if (!title) throw new Error("Enter a preset name first.");
    const values = captureValues(node.widgets || []);
    if (!Object.keys(values).length) throw new Error("This node has no scalar widget settings to save.");
    preferences.presets.push({id: crypto.randomUUID(), name: title, node_id: node.comfyClass || node.type, values});
    await savePreferences(); renderBody(); message("Preset saved.");
  }, "doss-primary"));
  for (const preset of preferences.presets.filter(item => `${item.name} ${item.node_id}`.toLowerCase().includes(query.toLowerCase()))) {
    ...
    card.append(button("Add with preset", () => addNode(preset.node_id, preset.values)), button("Apply to selected", () => {
      ...
      if ((node.comfyClass || node.type) !== preset.node_id) throw new Error("Select a node of the same type as this preset.");
      let missing: string[] = [];
      ...
      await savePreferences(); renderBody(); message("Preset removed; existing nodes are unchanged.");
    }));
    body.append(card);
  }
}
async function diagnosticsView() {
  ...
  const data = await request<any>("/doss/hub/status");
  if (!holder.isConnected) return;
  for (const [name, info] of Object.entries(data.dependencies) as [string, any][]) holder.append(el("p", "doss-health", `${info.available ? "✓" : "!"} ${name} · ${info.version || "Missing — reinstall the declared dependencies"}`));
  ...
  holder.append(button("Refresh installed model inventory", async () => {
    const inventory = await request<any>("/doss/hub/models"); models.replaceChildren();
    for (const [category, files] of Object.entries(inventory.models) as [string, string[]][]) {
      const section = el("details", "doss-category");
      section.append(el("summary", "", `${category} · ${files.length}`), el("pre", "doss-report", files.length ? files.join("\n") : "No installed models in this category.")); models.append(section);
    }
...
function aboutView() {
  ...
  for (const [label, url] of Object.entries(catalog.links)) {
    const link = el("a", "doss-button", label); link.href = url; link.target = "_blank"; link.rel = "noopener noreferrer"; links.append(link);
  }
...
function renderBody() {
  ...
  for (const tab of root?.querySelectorAll<HTMLButtonElement>(".doss-tab") || []) tab.setAttribute("aria-selected", String(tab.textContent === activeTab));
  if (activeTab === "Tools") toolsView();
  else if (activeTab === "Workflows") workflowsView();
  else if (activeTab === "Presets") presetsView();
  else if (activeTab === "Inspect") void diagnosticsView().catch(showError);
  else aboutView();
}
async function render(container: HTMLElement) {
  root = el("div", "doss-hub"); container.replaceChildren(root);
  const header = el("header", "doss-header");
  const brand = el("div"); brand.append(el("div", "doss-eyebrow", "YOUR CREATIVE TOOLKIT"), el("h1", "", "Doss Hub"));
  header.append(brand, el("span", "doss-version", "0.7"));
  const search = el("input", "doss-input doss-search"); search.type = "search"; search.placeholder = "Find a tool or a task…"; search.setAttribute("aria-label", "Search Doss tools"); search.value = query;
  ...
  const tabs = el("nav", "doss-tabs"); tabs.setAttribute("aria-label", "Doss Hub sections"); tabs.setAttribute("role", "tablist");
  for (const label of ["Tools", "Workflows", "Presets", "Inspect", "About"]) {
    const tab = button(label, () => { activeTab = label; renderBody(); }, "doss-tab");
    tab.setAttribute("role", "tab"); tab.onkeydown = (event) => {
      if (event.key !== "ArrowRight" && event.key !== "ArrowLeft") return;
      event.preventDefault(); const all = [...tabs.querySelectorAll<HTMLButtonElement>("button")];
      const next = (all.indexOf(tab) + (event.key === "ArrowRight" ? 1 : -1) + all.length) % all.length;
      all[next].focus(); all[next].click();
    }; tabs.append(tab);
  }
  statusLine = el("p", "doss-status", "Loading your tools…"); statusLine.setAttribute("role", "status"); statusLine.setAttribute("aria-live", "polite");
  body = el("div", "doss-body"); body.setAttribute("role", "tabpanel");
  root.append(header, search, favorite, tabs, statusLine, body);
  try {
    const [data, info, prefs] = await Promise.all([request<Catalog>("/doss/hub/catalog"), request<Record<string, unknown>>("/object_info"), request<Preferences>("/doss/hub/preferences")]);
    catalog = data; objectInfo = info; preferences = prefs;
    if (catalog.schema_version !== 1 || preferences.schema_version !== 1) throw new Error("Unsupported Doss data version. Update the suite and refresh.");
    message(`${catalog.nodes.length} tools · Ready`); renderBody();
  } catch (error) { showError(error); body.append(button("Retry", () => render(container))); }
}

app.registerExtension({
  name: "Doss.Hub",
  setup() {
    if (!app.extensionManager?.registerSidebarTab) { console.warn("Doss Hub requires ComfyUI's sidebar extension API. Backend nodes remain available."); return; }
    const style = el("link"); style.rel = "stylesheet"; style.href = new URL("./doss_hub.css", import.meta.url).href; document.head.append(style);
    app.extensionManager.registerSidebarTab({id: "doss-hub", icon: "pi pi-th-large", title: "Doss Hub", tooltip: "Doss tools, presets, examples, and diagnostics", type: "custom", render: (container: HTMLElement) => { void render(container); }});
  }
});
