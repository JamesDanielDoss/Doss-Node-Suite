...
function synchronizeType(node: any) {
  const type = node.widgets?.find((w: any) => w.name === "data_type")?.value;
  if (!type) return;
  for (const slot of node.inputs || []) {
    if (["a", "b", "batch"].includes(slot.name)) slot.type = type;
  }
...
app.registerExtension({
  ...
  beforeRegisterNodeDef(nodeType: any, data: any) {
    if (!data.name?.startsWith("Doss")) return;
    if (typedNodes.has(data.name)) {
      const created = nodeType.prototype.onNodeCreated;
      nodeType.prototype.onNodeCreated = function(...args: unknown[]) {
        const result = created?.apply(this, args);
        const widget = this.widgets?.find((w: any) => w.name === "data_type");
        if (widget) {
          const callback = widget.callback;
          widget.callback = (...values: unknown[]) => { callback?.apply(widget, values); synchronizeType(this); };
        }
        synchronizeType(this); return result;
      };
    ...
    nodeType.prototype.onExecuted = function(message: any) {
      ...
      if (curve) {
        ...
        let points: number[][] = curve.points;
        if (curve.interpolation === "step") points = points.flatMap((point, i) => i ? [[point[0], points[i-1][1]], point] : [point]);
        const polyline = document.createElementNS(svg.namespaceURI, "polyline");
        polyline.setAttribute("points", curvePolyline(points, 300, 100).map(p => p.join(",")).join(" "));
        polyline.setAttribute("fill", "none"); polyline.setAttribute("stroke", "#67d5bb"); polyline.setAttribute("stroke-width", "2");
        svg.append(polyline); this.dossReport.append(svg);
      }
      const text = document.createElement("pre"); text.style.whiteSpace = "pre-wrap"; text.style.margin = "0";
